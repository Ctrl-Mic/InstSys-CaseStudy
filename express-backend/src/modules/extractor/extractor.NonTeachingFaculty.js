...
class NonTeachingFacultyExtractor {
   ...
   */
  async extractNonTeachingFacultyExcelInfo(sheet) {
    try {
      ...
      const facultyInfo = this.extractUniversalFacultyData(allText);
   ...
   */
  extractUniversalFacultyData(textContent) {
...
PhilHealth: ${formatField(philhealth)}`;
   ...
   */
  async processNonTeachingFacultyExcel(sheet) {
    try {
      // EXTRACT
      const facultyInfo = await this.extractNonTeachingFacultyExcelInfo(sheet);
...
export default NonTeachingFacultyExtractor;