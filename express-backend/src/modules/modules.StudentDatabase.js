...
class StudentDatabase {
  constructor(connectionString = null, databaseName = 'school_system') {
    ...
    this.db = null;
    this.MediaDefaults = {
      IMAGE: { data: 'default_profile', filename: 'default_profile.jpg', path: '/images/default_profile.jpg' },
      AUDIO: { data: null, filename: null, path: null }
    };
    this.FieldStatus = { COMPLETE: 'complete', WAITING: 'waiting', MISSING: 'missing' };
  ...
  async createStudentRecord(data, source = 'file_extraction') {
    try {
      const studentDoc = {
        ...
        image: {
          ...
          status: source === 'file_extraction'
            ? this.FieldStatus.WAITING
            : (data.image_data ? this.FieldStatus.COMPLETE : this.FieldStatus.WAITING)
        },
        audio: {
          ...
          status: source === 'file_extraction'
            ? this.FieldStatus.WAITING
            : (data.audio_data ? this.FieldStatus.COMPLETE : this.FieldStatus.WAITING)
        },
      ...
      if (studentDoc.image.status === this.FieldStatus.WAITING ||
        studentDoc.audio.status === this.FieldStatus.WAITING) {
  ...
  _determineFieldStatus(data, source) {
    ...
    textFields.forEach(field => {
      ...
      if (source === 'manual_input') {
        fieldStatus[field] = value ? this.FieldStatus.COMPLETE : this.FieldStatus.WAITING;
      } else {
        fieldStatus[field] = value ? this.FieldStatus.COMPLETE : this.FieldStatus.MISSING;
      }
    });

    fieldStatus.image = this.FieldStatus.WAITING;
    fieldStatus.audio = this.FieldStatus.WAITING;
  ...
  async _addToPendingMedia(studentDoc) {
    const pendingDoc = {
      ...
      waiting_for: {
        image: studentDoc.image.status === this.FieldStatus.WAITING,
        audio: studentDoc.audio.status === this.FieldStatus.WAITING
      },
      added_at: new Date()
    };
  ...
  async updateMedia(studentId, mediaType, mediaData, filename, department) {
    try {
      ...
      const updateData = {
        ...
        [`${mediaType}.status`]: this.FieldStatus.COMPLETE,
        [`field_status.${mediaType}`]: this.FieldStatus.COMPLETE,
        updated_at: new Date()
      };
  ...
  async _updateCompletionPercentage(studentId, department) {
    ...
    if (student.image?.status === this.FieldStatus.COMPLETE) completed++;
    if (student.audio?.status === this.FieldStatus.COMPLETE) completed++;
  ...
  async _checkPendingMediaComplete(studentId, department) {
    ...
    const imageComplete = student.image?.status === this.FieldStatus.COMPLETE;
    const audioComplete = student.audio?.status === this.FieldStatus.COMPLETE;
  ...
  getStudentDisplay(student) {
    ...
    const imageIsEmpty = !displayStudent.image?.data ||
      ...
      displayStudent.image?.status === this.FieldStatus.WAITING;

    if (imageIsEmpty) {
      displayStudent.image = {
        data: this.MediaDefaults.IMAGE.data,
        filename: this.MediaDefaults.IMAGE.filename,
        display_path: this.MediaDefaults.IMAGE.path,
        status: displayStudent.image?.status || this.FieldStatus.WAITING,
        is_default: true
      };
    } else {
    ...
    const audioIsEmpty = !displayStudent.audio?.data ||
      ...
      displayStudent.audio?.status === this.FieldStatus.WAITING;

    if (audioIsEmpty) {
      displayStudent.audio = {
        data: this.MediaDefaults.AUDIO.data,
        filename: this.MediaDefaults.AUDIO.filename,
        display_path: this.MediaDefaults.AUDIO.path,
        status: displayStudent.audio?.status || this.FieldStatus.WAITING,
        is_default: true
      };
    } else {
